<template>
	<view class="container">
		...
		<view class="history-coupon-box" @tap="jumpHistoryCoupon">查看历史优惠券</view>
...
export default {
	...
	onLoad(options) {
		...
		if (options.source == 0) {
			this.couponParameter.source = 0;
			this.couponParameter.usedAmount = options.usedAmount * 1;
		}
		if (options.source == 1) {
			this.couponParameter.source = 1;
			this.couponParameter.usedAmount = options.usedAmount * 1;
			this.couponParameter.shopId = prevPage.$vm.orderPara.shopId;
		}
		if (prevPage.$vm.share == 1 ? true : false) this.couponParameter.isShare = 0;
		this.couponParameter.storeId = prevPage.$vm.orderPara.storeId;
		this.couponParameter.shopId = prevPage.$vm.orderPara.shopId;
		this.couponParameter.saleType = prevPage.$vm.tabtype ? 0 : 1;
	},
	onShow() {
		this.couponList = [];
		this.totalAmt = 0;
	...
	methods: {
		...
		init() {
			...
			let successFuc = res => {
				...
				const prevPage = thisPages[thisPages.length - 2]; //上一个页面
				if (!res?.data.length) {
					this.$refs.uTips.show({
						title: "暂无优惠券",
						duration: "1500",
				...
				this.couponList = this.couponList.concat(
					res.data.map(it => {
						if (prevPage.$vm.orderPara?.redPacketIds.findIndex(ele => ele === it.recordId) == -1) {
							it.active = false;
						} else {
							it.active = true;
							this.count = 1;
			...
			this.couponParameter.source == 0
				? AjaxApi("SelectUserRedPacketList_01", this.couponParameter, successFuc)
				: AjaxApi("SelectRedPacket_shop", this.couponParameter, successFuc);
		},
		tapView(i) {
			if (Boolean(this.couponList[i].isPay)) return;
			const pages = getCurrentPages();
			const prevPage = pages[pages.length - 2]; //上一个页面
			const inx = prevPage.$vm.orderPara.redPacketIds.findIndex(item => item == this.preId);
			this.couponList = this.couponList.map((item, index) => {
				if (i == index) {
					item.active = !item.active;
					if (inx !== -1) {
						prevPage.$vm.orderPara.redPacketIds[inx] = null;
						prevPage.$vm.orderPara.redPackageFee[inx] = 0;
					}
					if (item.active) {
						this.preId = item.recordId;
						this.couponParameter.source == 0
							? (prevPage.$vm.orderPara.redPackageFee[0] = item.amount)
							: (prevPage.$vm.orderPara.redPackageFee[1] = item.amount);
						this.couponParameter.source == 0
							? (prevPage.$vm.orderPara.redPacketIds[0] = item.recordId)
							: (prevPage.$vm.orderPara.redPacketIds[1] = item.recordId);
					} else {
						this.couponParameter.source == 0 ? (prevPage.$vm.orderPara.redPackageFee[0] = 0) : (prevPage.$vm.orderPara.redPackageFee[1] = 0);
						this.couponParameter.source == 0 ? (prevPage.$vm.orderPara.redPacketIds[0] = null) : (prevPage.$vm.orderPara.redPacketIds[1] = null);
					}
				} else {
					item.active = false;
				}
		...
		jumpHistoryCoupon() {
			uni.navigateTo({
				url: `/pagesB/historicalCoupon/historicalCoupon`,
			});
		},
	},
};
