<template>
	<view class="wrapper">
		...
		<view class="spell-box">
			<view class="address-box" @tap="jumpAddressList">
				<view class="address-left">
					<view :class="{ none: !Boolean(groupMsg.address) }">{{ groupMsg.address || "请选择收货地址" }}</view>
					<view>
			...
			<view class="purchase u-m-l-24 u-m-r-24" v-if="groupMsg.addBuyList.length">
				...
				<view class="good u-m-b-20" v-for="(it, i) in groupMsg.addBuyList" :key="i">
					...
					<view style="margin-left: auto" class="u-flex u-flex-nowrap">
						<image v-if="!it.number" class="img-c" src="https://oss.dianddian.cn/v2/purchase/minus_none.png" mode="aspectFill"></image>
						<image
							v-else
							class="img-c"
						...
						<image class="img-c" src="https://oss.dianddian.cn/v2/purchase/add.png" mode="aspectFill" @tap.stop="addPurchase(it, true)"></image>
					</view>
				</view>
			</view>
		</view>
		<view class="instal-box">
			...
			<view class="info-box" v-if="groupMsg.create && groupMsg.platformRedPacket">
			...
			<view class="info-box" v-if="groupMsg.reduceFee && groupMsg.merchantReduce">
...
export default {
	...
	onShareAppMessage: function (res) {
		...
		if (res.from === "button") {
			const cbdId = that.groupMsg.id;
			const activityId = that.groupMsg.activityId;
			// 来自页面内转发按钮
			return {
				title: `三十分钟准时达`,
				imageUrl: "https://oss.dianddian.cn/share/canbida.png",
				path: `/pagesCan/canBida/canBida?cbdId=${cbdId}&activityId=${activityId}`,
			};
		} else {
			return {
				title: "进入餐典到了",
	...
	onLoad(query) {
		this.cbdId = !query?.id ? this.cbdId : query.id;
		this.pageType = !query?.type ? this.pageType : query.type * 1;
	},
	onHide() {
		//关闭
		if (getApp().globalData.conectSocket == true) {
			getApp().globalData.conectSocket = false;
			getApp().linkWebsocketCan();
		}
	},
	onUnload() {
		//关闭
	...
	onShow() {
		...
		let successFuc = async res => {
			let ACT = await store.dispatch("CanActDetail", { id: res.data.activityId });
			let { platformRedPacket, merchantReduce } = ACT.data;
			this.groupMsg = { ...res.data, platformRedPacket, merchantReduce };
		...
		AjaxApi("CanSettle", params, successFuc, elseFuc);
	},
	methods: {
		jumpMember() {
			uni.navigateTo({
		...
		jumpAddressList() {
			...
			//选择收货地址
			uni.navigateTo({
				url: `/pagesB/addressList/addressList?activityId=${this.groupMsg.activityId}&storeId=${this.groupMsg.storeId}`,
			});
		},
		socketFuc() {
			if (this.groupMsg.id) {
				...
				getApp().linkWebsocketCan(`${this.groupMsg.id}/${getApp().globalData.UserInfo.userId}`);
				getApp().initSocket();
			} else {
				//关闭
				getApp().globalData.conectSocket = false;
		...
		async socketFresh() {
			let groups = await store.dispatch("CanDetail", { cbdId: this.cbdId });
			if (groups.code == 0) {
				let ACT = await store.dispatch("CanActDetail", { id: this.groupMsg.activityId });
				let { platformRedPacket, merchantReduce } = ACT.data;
				this.groupMsg = { ...groups.data, platformRedPacket, merchantReduce };
		...
		doDisJion() {
			...
			AjaxApi("CanCancel", params, successFuc);
		},
	},
	computed: {
		CALC() {
