		...
		<view class="item-con">
			<view class="item-box">
				<view class="item" v-for="(it, i) in couponList" :key="i">
					<view class="redEnvelope-left" :style="{ background: it.source == 0 ? '#FFF4ED' : '#FFF1F1', color: it.source == 0 ? '##FE6B00' : '#FF3B3B' }">
					...
					<view class="redEnvelope-right">
						<view>
							<view class="redEnvelope-name">{{ it.redPacketName }}</view>
							<view class="redEnvelope-time">{{ !it.type ? "长期有效" : getCurrent(it.validRemark) }}</view>
						</view>
						<view class="redEnvelope-btn" :style="{ background: it.source == 0 ? '#FE6B00' : '#FF3B3B' }">去使用</view>
						<view class="redEnvelope-tag" v-if="it.isShare == 0">同享券</view>
					</view>
				</view>
			</view>
		</view>
